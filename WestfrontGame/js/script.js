...
function selectRandomImages(allImages) {
    ...
    for (let i = imageSelection.length - 1; i > 0; i--) {
      ...
      [imageSelection[i], imageSelection[j]] = [imageSelection[j], imageSelection[i]];
    }
...
function loadNewImage(randomImages, index) {
    ...
    document.getElementById('bigImage').replaceWith(imageElement);
    secondWindow.document.getElementById("testDiv").innerHTML = "<p>Hello World</p>";
}
